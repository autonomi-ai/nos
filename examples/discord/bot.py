...
async def setup():
    """Initialize all threads in the NOS_PLAYGROUND_CHANNEL"""
    logger.debug("initializing all threads")
    for channel in bot.get_all_channels():
        if channel.name == NOS_PLAYGROUND_CHANNEL:
            for thread in await channel.threads():
                THREADS[thread.name] = thread.id
    logger.debug(f"initialized all threads [threads={THREADS}]")


@bot.command()
async def generate(ctx, *, prompt):
    """Create a callback to read messages and generate images from prompt"""
    logger.debug(f"/generate [prompt={prompt}, channel={ctx.channel.name}, channel_id={ctx.channel.id}, user={ctx.author.name}]")

    if ctx.channel.name != NOS_PLAYGROUND_CHANNEL:
        logger.debug(f"ignoring [channel={ctx.channel.name}]")
        return
    ...
    image_bytes.seek(0)
    await ctx.send(f"{prompt}", file=discord.File(image_bytes, filename=f"{ctx.message.id}.png"))
...
async def train(ctx, *, prompt):
    ...
    thread_id = str(ctx.message.id)
    thread = await ctx.channel.create_thread(name=thread_id, type=discord.ChannelType.public_thread)
    await thread.send(f"Created a new thread for training [id={thread.name}]")

    # Create the training directory for this thread
    dirname = NOS_TRAINING_VOLUME_DIR / thread_id
